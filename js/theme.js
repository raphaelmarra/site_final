    ...
    s(document).ready(function() {
        ...
        function c(e, l) {
            if (document.querySelector(e) != null) {
                s(".shipping-result").html(s(".page-simula-frete"));
                return
            } else {
                setTimeout(function() {
                    c(e, l)
        ...
        s(".botao-simular-frete, #shippingSimulatorButton").click(function() {
            s(".shipping-result").html("");
            c(".page-simula-frete", 1e3)
        });
        if (s.fn.jquery != "1.6.2") {
            s(".banner-home-slide").slick({
                slidesToShow: 1,
                slidesToScroll: 1,
            ...
            s(".page-home .showcase-list").slick({
                lazyLoad: "ondemand",
                slidesToShow: 4,
                slidesToScroll: 4,
                dots: false,
        ...
        if (s(".caixa-cupom").length) {
            s(".caixa-cupom").parents("tr").addClass("cupom-wrapper")
        }
        if (s("#calculoFrete").length) {
            s("#calculoFrete").parents("tr").addClass("frete-wrapper")
        }
        s(".open-filters").click(function() {
            s(".filters-list").slideToggle()
        })
    });
    var e = s(".floating");
    s(window).scroll(function() {
        if (s(window).scrollTop() > 212) {
            e.addClass("fixed")
        } else {
            e.removeClass("fixed")
        }
    });
    s(".modal-login").click(function(e) {
        e.preventDefault();
        s("tray-login").show()
    })
}
...
(function() {
    ...
    var t = document.getElementsByClassName("menu-mobile-backdrop")[0];
